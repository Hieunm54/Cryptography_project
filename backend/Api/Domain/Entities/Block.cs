...
namespace APITemplate.Domain.Entities
{
    public class Block
    {
        ...
        public Block(DateTime DateCreate, string PreviousHash)
        {
            this.DateCreate = DateCreate;
            //this.Data = Data;
            this.PreviousHash = PreviousHash;
            this.Nonce = 0;
            this.Hash = TinhToanHash();
            this.ListExchange = new List<Exchange>();
            this.Value = 100;
        }
        public string TinhToanHash() { //Hàm mã hóa nội dung của toàn bộ Block. Do đó ta cần lấy toàn bộ các thuộc tính của Block đưa vào SHA256 để mã hóa ra một chuỗi.
            SHA256 sha256Hash = SHA256.Create();
            var result = sha256Hash.ComputeHash(Encoding.UTF8.GetBytes(this.Hash + this.ListExchange + this.DateCreate + this.Nonce));
